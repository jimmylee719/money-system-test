...
**非 OpenAPI 目錄的官方端點**（如 TWSE 網站的 T86 個股三大法人，目錄查無替代品）
仍屬一手來源，但須標記 `endpointStability: 'website_internal'`，drift 監控加強。
**網址需要日期參數時，日期取自其他來源 payload 宣告的交易日，不用系統時鐘推定**
——推定會撞到週末、國定假日、颱風假。取不到就不抓，並記錄失敗原因。
