...
export type SourceId =
  ...
  | 'tpex_exright_daily'
  // ───────────────────────────────────────────────────────────────────────
  // P11.15：擴充公部門來源。**除了停資停券進 L2 外，其餘一律只累積不使用**。
  //
  // 為什麼現在抓、卻現在不用：因子必須在**看到結果之前**登記（CLAUDE.md），
  // 而檢定需要歷史。現在不抓，P12 一樣沒有資料可檢定。
  // 反過來說，若在看過 2026-08 的觀察榜表現之後才挑因子，那就是前視偏誤。
  // 所以：資料現在累積，因子等 P12 依規矩登記。
  // ───────────────────────────────────────────────────────────────────────
  /** L2 否決：停資停券預告。抽掉槓桿資金，常伴隨劇烈波動 */
  | 'twse_margin_suspension'
  /** G3 基準：加權股價報酬指數（含息），比 0050 ETF 乾淨（無折溢價、無管理費） */
  | 'twse_taiex_total_return'
  /** 借券可賣出股數。台股機構放空主要走借券不走融券，只看融券會漏掉法人空單 */
  | 'twse_sbl_available'
  /** 營益分析：毛利率／營益率／純益率。只看營收會漏掉「賠本搶單」 */
  | 'twse_profitability'
  /** 董監持股與**設質比例**。高質押在下跌時觸發追繳，形成自我強化的螺旋 */
  | 'twse_director_holdings'
  /** 273 個類股指數。個股報酬有相當比例來自類股，不看類股等於少看一半 */
  | 'twse_sector_indices'
  /** 集中市場每日成交金額與加權指數 */
  | 'twse_market_turnover'
  /** 期貨每日行情。與現貨指數相減得期現價差（逆價差＝法人避險或看空） */
