...
/**
 * 【2026-08-16 實測觀察 — 只記錄，不得據此修改定義】
 * 上櫃 820 檔有融資餘額者當中，138 檔的前日餘額 < 100 張。
 * 餘額基數小的時候，這個比率極不穩定：6961 由 5 張變 0 張就是 −100%，
 * 6615 由 4 張變 1 張是 −75%。這些數字是官方原值，不是解析錯誤（已逐筆核對）。
 *
 * 定義已鎖定，**不得事後補上最小餘額門檻**——那正是回測後調參。
 * 處理方式有二，都不動這個因子：
 *   1. 前瞻檢定會如實反映。若雜訊蓋過訊號，t 值到不了 3.0，屆時封存即可。
 *   2. 流動性與規模的過濾屬 L2 否決層（P6），該層只能減少行動，加了不會造成偏誤。
 * 另外，等權合成本身已大幅稀釋單一因子的雜訊：實測 Top 5 的融資變動為
 * −47%／−18%／−5.8%／−5.4%／−5.5%，並未被微量餘額的極端值佔據。
 */
