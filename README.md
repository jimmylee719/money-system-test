...
## P5a：L1 正規化與標的池（`src/lib/l1`）

L0 是「官方原話」，L1 是「解讀後的事實」。所有解讀都在這一層，且必須顯式、可測、可統計。

### 標的池用公司名冊，不用代號正則

直覺做法是「4 碼純數字＝股票」。實測證明這是錯的：

| 反例 | 後果 |
|---|---|
| `910322`、`911608` 等 6 檔第一上市外國公司是 6 碼 | 正則會**漏掉** |
| `0050` 是 4 碼純數字但它是 ETF 不是公司 | 正則會**誤收** |
| 上櫃行情表 10,489 列中 9,600 列非公司標的（權證等） | 不過濾就會拿權證去排序 |

唯一依據是交易所自己維護的公司名冊（`mops_*_company_profile`）。實測結果：

```
上市：名冊 1095 檔｜行情表 1378 筆｜交集 1093｜排除 285｜可交易 1091
上櫃：名冊  890 檔｜行情表 10489 筆｜交集  889｜排除 9600｜可交易  869
合計標的池 1985 檔
```

### 缺值不得代換為 0

**沒有本益比與本益比為 0 是完全不同的兩件事**——代換為 0 會讓虧損公司在
「本益比越低越好」的排序裡排第一。所有缺值一律為 `null`。

官方的缺值表示（全部來自實測）：

| 樣態 | 出現在 |
|---|---|
| `""` | 上市本益比／殖利率（虧損公司），實測 449 個欄位值 |
| `" ---"` | 上櫃行情，當日無成交 |
| `"N/A"` | 上櫃評價，實測 218 筆 |
| `"-"` `"--"` | 各處 |
...
上櫃行情的漲跌欄在除權息日放的是**文字**（`"除權 "`、`"除息 "`）。
直接當缺值丟掉會遺失資訊——**P9 計算報酬時必須把除權息還原，丟掉會低估報酬**。
故 `DailyQuote.changeNote` 逐字保留。

未知的非數值仍計入 `unparsable`，**新的格式變動不會被吞掉**。
...
用真實 L0 快照驗證：讀取時雜湊比對、**解析零失敗**、標的池數字。
`unparsable > 0` 即代表官方格式變了，必須有人看。

> 這支驗證在第一次執行時就抓到兩個樣本測試沒涵蓋的真實格式
> （上櫃的 `N/A` 與除權息註記）——測試通過不等於測對了東西。
